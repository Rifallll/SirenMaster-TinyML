"""
audit_dan_bersihkan_ambulance.py
================================
Audit Akustik Khusus Dataset AMBULANCE:
1. Mendeteksi file yang berisi suara manusia bicara, hening, atau bukan sirine wail.
2. Memindahkan file-file yang tidak sesuai ke folder: KARANTINA_AMBULANCE_NON_WAIL/
   (File TIDAK langsung dihapus permanen, melainkan dipisahkan agar bisa direview).
3. Memastikan hanya file sirine Ambulans murni berkualitas tinggi yang tersisa di folder AMBULANCE/.
"""
...
QUARANTINE_DIR = os.path.join(ROOT, "KARANTINA_AMBULANCE_NON_WAIL")
MODEL_PATH = os.path.join(ROOT, "siren_classifier_model.h5")

os.makedirs(QUARANTINE_DIR, exist_ok=True)

print("=" * 70)
print("🔍 AUDIT & PEMBERSIHAN DATASET AMBULANS")
print(f"📁 Folder Sumber     : {AMB_DIR}")
print(f"📁 Folder Karantina  : {QUARANTINE_DIR}")
print("=" * 70)

# 1. Load model jika ada untuk verifikasi silang
model = None
...
wav_files = sorted(glob.glob(os.path.join(AMB_DIR, "*.wav")))
...
for idx, filepath in enumerate(wav_files):
    ...
    # 2. Cek Karakteristik Akustik Sirine (Dominasi frekuensi 500 - 2500 Hz)
    fft_mag = np.abs(np.fft.rfft(y))
    freqs = np.fft.rfftfreq(len(y), 1.0 / sr)
    
    siren_band = (freqs >= 450) & (freqs <= 3000)
    low_band = (freqs < 450)
    ...
    # Jika suara didominasi frekuensi rendah (suara orang bicara/gumaman, gemuruh mesin)
    # Suara manusia bicara biasanya memiliki energi dominan di < 400 Hz
    if siren_ratio < 0.35 and (low_energy / total_energy) > 0.55:
        quarantine_list.append((fname, f"Bukan Sirine / Suara Manusia-Mesin (Siren Ratio={siren_ratio*100:.1f}%)", filepath))
        continue
        
    # 3. Verifikasi dengan Model AI
    if model is not None and mean_c is not None:
        ...
        # Jika AI sangat yakin ini NORMAL (> 70%) dan skor Ambulans < 15%
        if preds[2] > 0.70 and preds[0] < 0.15:
...
print("=" * 70)
print(f"📊 HASIL AUDIT DATASET AMBULANS:")
...
# Pindahkan file tidak sesuai ke karantina
...
print("\n🎯 Proses Selesai! File bersih tetap di AMBULANCE/, file tidak sesuai ada di KARANTINA_AMBULANCE_NON_WAIL/.")
