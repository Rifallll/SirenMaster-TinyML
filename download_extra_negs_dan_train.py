"""
download_extra_negs_dan_train.py
===========================
1. Download suara-suara rawan tambahan (Jingle es krim, hujan, palang kereta, rem truk, anjing)
...
DOWNLOAD_DIR = r"C:\Users\ASUS\Videos\DATASET\TRASH\extranegs_download"
os.makedirs(DOWNLOAD_DIR, exist_ok=True)
os.makedirs(NORMAL_DIR, exist_ok=True)

# ── Daftar pencarian YouTube untuk tambahan suara pengecoh ──
SEARCH_QUERIES = [
    "ytsearch1:suara lagu es krim keliling walls campina",
    "ytsearch1:suara hujan lebat deras dengan petir",
    "ytsearch1:suara sirine palang pintu kereta api",
    "ytsearch1:suara rem angin desis truk bus",
    "ytsearch1:suara anjing menggonggong keras"
]

print("="*65)
print(" DOWNLOADER SUARA PENGECOH TAMBAHAN (FINAL ROAD TEST)")
print("="*65)
...
for i, query in enumerate(SEARCH_QUERIES):
    ...
    out_tpl = os.path.join(DOWNLOAD_DIR, f"extraneg_{i+1:02d}.%(ext)s")
    ...
    found = glob.glob(os.path.join(DOWNLOAD_DIR, f"extraneg_{i+1:02d}.*"))
    if found:
...
for fpath in downloaded:
    ...
    try:
        ...
        for s in range(n_seg):
            ...
            out_name = os.path.join(NORMAL_DIR, f"extra_noise_{base}_seg{s+1:04d}_{ts}.wav")
...
print(f"\n Total segmen suara pengecoh tambahan baru: {total_segments} file di folder NORMAL")
...
if total_segments > 0:
    ...
    print("\n ✅ SELESAI! Model ULTIMATE jalanan siap.")
