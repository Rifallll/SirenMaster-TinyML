...
def audit_and_clean():
    print("="*60)
    print("   MEMULAI AUDIT LENGKAP & MEMBERSIHKAN FILE SALAH FOLDER")
    print("="*60)
    ...
    # Clean mismatches directly in memory
    print("\n" + "="*50)
    print("               AUDIT & CLEAN REPORT")
    print("="*50)
    print(f"Total Audited Files: {total_files}")
    print(f"Mismatches found   : {len(mismatches)}")
    
    if len(mismatches) > 0:
        print(f"\n[WARN] Memindahkan {len(mismatches)} file salah folder / mencurigakan ke folder TRASH...")
        moved_count = 0
        for t, p, fname, conf, path in mismatches:
            if os.path.exists(path):
                dest_path = os.path.join(TRASH_DIR, fname)
                
                # Handle unique name if file exists in TRASH
                ...
                try:
                    ...
                    print(f"  [MOVED] {fname} (true: {t} | pred: {p} {conf*100:.1f}%) -> TRASH")
                    moved_count += 1
                except Exception as e:
                    print(f"  [ERROR] Gagal memindahkan {fname}: {e}")
