"""
audit_dan_bersihkan_damkar.py
=============================
Audit Akustik Khusus Dataset FIRETRUCK (DAMKAR):
1. Mendeteksi file yang berisi suara manusia bicara, hening, atau bukan sirine Damkar.
2. Memisahkan & menghapus file-file yang tidak sesuai.
3. Memastikan hanya file sirine Damkar murni (Q-Siren, Electronic Wail, Air Horn) yang tersisa.
"""
...
FIRE_DIR = os.path.join(ROOT, "FIRETRUCK")
QUARANTINE_DIR = os.path.join(ROOT, "KARANTINA_FIRETRUCK_NON_DAMKAR")
MODEL_PATH = os.path.join(ROOT, "siren_classifier_model.h5")

os.makedirs(QUARANTINE_DIR, exist_ok=True)

print("=" * 70)
print("🚒 AUDIT & PEMBERSIHAN DATASET DAMKAR (FIRETRUCK)")
print(f"📁 Folder Sumber     : {FIRE_DIR}")
...
wav_files = sorted(glob.glob(os.path.join(FIRE_DIR, "*.wav")))
...
for idx, filepath in enumerate(wav_files):
    ...
    # 2. Cek Karakteristik Akustik Sirine Damkar (400 - 3200 Hz)
    fft_mag = np.abs(np.fft.rfft(y))
    freqs = np.fft.rfftfreq(len(y), 1.0 / sr)
    
    siren_band = (freqs >= 400) & (freqs <= 3200)
    low_band = (freqs < 400)
    ...
    # Jika suara didominasi vokal/gumaman orang bicara (< 400 Hz)
    if siren_ratio < 0.30 and (low_energy / total_energy) > 0.60:
        quarantine_list.append((fname, f"Bukan Sirine Damkar / Suara Manusia-Mesin (Siren Ratio={siren_ratio*100:.1f}%)", filepath))
        continue
    ...
    if model is not None and mean_c is not None:
        ...
        # Jika AI sangat yakin ini NORMAL (> 75%) dan Damkar < 12%
        if preds[2] > 0.75 and preds[1] < 0.12:
...
print("=" * 70)
print(f"📊 HASIL AUDIT DATASET DAMKAR (FIRETRUCK):")
